...
function resolveProduct(slug: string): Product {
  ...
  if (!product) {
    throw new Error(`未找到货号为 ${slug} 的商品`);
  }
  return product;
}
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const applyGiftCard = useCallback((code: string) => {
    ...
    if (!amount) {
      return { success: false, message: "礼品卡无效" };
    }
    setGiftCard({ code: normalized, amount });
    return { success: true, message: `礼品卡已使用：CHF ${amount.toFixed(2)}` };
  }, []);
  ...
  const addAddress = useCallback(
    (address: Omit<Address, "id" | "isDefault"> & { isDefault?: boolean }) => {
      ...
      return { success: true, message: "地址已保存", id };
    },
    []
  );
  ...
  const placeOrder = useCallback(
    ({ addressId, paymentMethodId, shippingSpeed }: {
      ...
      if (!bagLines.length) {
        return { success: false, message: "购物袋为空" };
      }
      ...
      if (!addressExists || !paymentExists) {
        return { success: false, message: "请选择有效的地址和支付方式" };
      }
      ...
      return { success: true, message: "订单提交成功", orderId };
    },
